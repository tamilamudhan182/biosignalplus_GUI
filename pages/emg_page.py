...
class EMGPanel(QWidget):
    def __init__(self):
        ...
        self.lowcut_input = QLineEdit("20.0")
        ...
        self.highcut_input = QLineEdit("450.0")
        ...
        # PLOTS SETUP (EMG & FFT)
        # ====================================
        self.emg_plot = pg.PlotWidget(title="EMG Signal")
        self.fft_plot = pg.PlotWidget(title="EMG PSD Spectrum")
        
        # Time Domain Plot Configuration
        plot_item = self.emg_plot.getPlotItem()
        ...
        fft_plot_item.setXRange(0, 500.0, padding=0)
        ...
        # Curves
        self.emg_raw_curve = plot_item.plot(pen=pg.mkPen('#888888', width=1), name="EMG Raw")
        self.emg_curve = plot_item.plot(pen=pg.mkPen('#00e676', width=1.5), name="EMG Filtered")
        
        # FFT Curve
        self.fft_curve = fft_plot_item.plot(pen=pg.mkPen('#ff9100', width=1.5), name="EMG FFT")
        ...
        plots_layout.addWidget(self.emg_plot, stretch=7)
        ...
        self.lbl_rms = QLabel("RMS: -- uV")
        self.lbl_rms.setStyleSheet(label_style)
        
        self.lbl_mav = QLabel("MAV: -- uV")
        self.lbl_mav.setStyleSheet(label_style)
        
        self.lbl_iemg = QLabel("IEMG: -- mV·s")
        self.lbl_iemg.setStyleSheet(label_style)
        
        self.lbl_med_freq = QLabel("Median Freq: -- Hz")
        self.lbl_med_freq.setStyleSheet(label_style)
        
        self.lbl_state = QLabel("Muscle State: --")
        self.lbl_state.setStyleSheet(label_style)
        
        feat_layout.addWidget(self.lbl_rms)
        feat_layout.addWidget(self.lbl_mav)
        feat_layout.addWidget(self.lbl_iemg)
        feat_layout.addWidget(self.lbl_med_freq)
    ...
    def set_raw_mode(self):
        self.show_mode = "raw"
        self.emg_curve.setVisible(False)
        self.emg_raw_curve.setVisible(True)

    def set_filtered_mode(self):
        self.show_mode = "filtered"
        self.emg_curve.setVisible(True)
        self.emg_raw_curve.setVisible(True)
    ...
    def update_auto_zoom(self, raw, filtered):
        try:
            ...
            self.emg_plot.getPlotItem().setYRange(ymin - margin, ymax + margin, padding=0)
        except Exception as e:
            print("EMG zoom error:", e)
            
    def update_fft(self, filtered, fs=1000.0):
        try:
            n = len(filtered)
            if n < 2:
                return
            f, psd = welch(filtered, fs=fs, nperseg=min(n, 1024))
    ...
    def set_theme(self, theme):
        ...
        self.emg_plot.setBackground(bg)
        self.fft_plot.setBackground(bg)
        
        self.emg_plot.getPlotItem().getAxis('left').setPen(fg)
        self.emg_plot.getPlotItem().getAxis('left').setTextPen(fg)
        self.emg_plot.getPlotItem().getAxis('bottom').setPen(fg)
        self.emg_plot.getPlotItem().getAxis('bottom').setTextPen(fg)
        ...
        self.emg_plot.getPlotItem().titleLabel.setAttr('color', fg)
        ...
        self.lbl_rms.setStyleSheet(text_style)
        self.lbl_mav.setStyleSheet(text_style)
        self.lbl_iemg.setStyleSheet(text_style)
        self.lbl_med_freq.setStyleSheet(text_style)
        self.lbl_state.setStyleSheet(text_style)

    def reconfigure_plots(self, fs):
        window_size = 300
        self.emg_plot.getPlotItem().setXRange(0, window_size, padding=0)
